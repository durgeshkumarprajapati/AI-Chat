...
import { CacheKeyOptions, ExactCacheItem, SemanticCacheItem } from './rag-cache.types';
...
export function generateExactCacheKey(opts: CacheKeyOptions): string {
  ...
  const rawString = `${opts.userId}|${opts.knowledgeBaseId || 'global'}|${opts.model || 'default'}|${opts.contextSummary || ''}|${normQuery}`;
