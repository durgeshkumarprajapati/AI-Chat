...
export default function KnowledgeBasesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      {/* Toolbar Controls */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-lg text-xs">
        <div className="relative w-full sm:w-80">
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 pl-9 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
          />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-4 rounded-2xl bg-slate-900/50 border border-slate-800/80">
          <span className="text-4xl">📚</span>
          <h3 className="text-lg font-bold text-white">No Knowledge Bases Found</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((kb) => (
            <div
              key={kb.id}
              className="rounded-2xl bg-slate-900 border border-slate-800 hover:border-slate-700 p-6 flex flex-col justify-between space-y-5 shadow-xl transition-all hover:shadow-2xl hover:shadow-indigo-950/20 group"
            >
              ...
              <div className="space-y-4 pt-3 border-t border-slate-800/80">
                <div className="grid grid-cols-3 gap-2 text-center text-xs font-mono">
                  <div className="p-2 rounded-xl bg-slate-950 border border-slate-800">
                    ...
                    <span className="text-slate-200 font-bold">{kb.documentCount}</span>
                  </div>
                  <div className="p-2 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-slate-500 block text-[10px]">Completed</span>
                    <span className="text-emerald-400 font-bold">{kb.completedDocuments}</span>
                  </div>
                  <div className="p-2 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-slate-500 block text-[10px]">Chunks</span>
                    <span className="text-indigo-400 font-bold">{kb.totalChunks}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <Link
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-indigo-600 text-white text-xs font-semibold transition-all"
                  >
                    Manage KB →
                  </Link>
                </div>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-slate-800 pt-6 text-xs text-slate-400">
          <div>
            ...
            <strong className="text-slate-200">{pagination.totalPages}</strong> ({pagination.total} total)
          </div>
          <div className="flex items-center space-x-2">
            <button
              disabled={page <= 1}
          ...
          <div className="max-w-md w-full rounded-2xl bg-slate-900 border border-slate-800 p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="text-slate-400 font-medium">Knowledge Base Name *</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-slate-400 font-medium">Description (Optional)</label>
                <textarea
      ...
      {isEditModalOpen && selectedKb && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-md w-full rounded-2xl bg-slate-900 border border-slate-800 p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">Edit Knowledge Base</h3>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-md w-full rounded-2xl bg-slate-900 border border-slate-800 p-6 space-y-4 shadow-2xl">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Are you sure you want to delete <strong className="text-white">&quot;{selectedKb.name}&quot;</strong>?
              This will remove the collection grouping. <strong className="text-emerald-400">All member documents, vector embeddings, chunks, and storage files will remain completely intact.</strong>
            </p>
            <div className="flex items-center justify-end space-x-3 pt-2">
              <button
