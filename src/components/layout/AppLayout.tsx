...
function InnerAppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/80 px-4 lg:px-8 py-3 flex items-center justify-between">
            ...
            {navItems.slice(0, 7).map((item) => {
              ...
              return (
                ...
                >
                  <span>{item.name}</span>
                  {item.badge && (
                    <span className="text-[10px] px-1.5 py-0.2 bg-indigo-950 text-indigo-400 rounded border border-indigo-800">
                      {item.badge}
                    </span>
                  )}
                </Link>
        ...
        <div className="flex items-center space-x-3">
          ...
          <Link
            href="/health"
            className="hidden sm:flex items-center space-x-2 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-xs font-medium hover:border-slate-700 transition-colors"
          >
            <span
