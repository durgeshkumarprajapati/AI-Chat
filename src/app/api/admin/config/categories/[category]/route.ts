...
export async function GET(req: NextRequest, { params }: { params: { category: string } }) {
  try {
    ...
    const category = params.category.toUpperCase() as ConfigCategory;
    ...
    return NextResponse.json(
      { success: false, error: { code: 'INTERNAL_SERVER_ERROR', message: 'Failed to retrieve category configurations' } },
      { status: 500 }
    );
  }
}
